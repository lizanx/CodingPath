...
public static class Components
{
    ...
    public static IHtmlContent TodoList(IEnumerable<TodoItem> todoItems)
    {
        ...
        {
            builder.AppendHtml(TodoItem(item));
        }
    ...
    {
        string isCompleted = todoItem.IsCompleted ? "checked" : string.Empty;
        var builder = new HtmlContentBuilder();
        builder.AppendHtml("<div>");
        builder.AppendHtml("<p>");
        builder.AppendFormat("{0}", todoItem.Name);
        builder.AppendHtml("</p>");
        builder.AppendHtml($"<input type=\"checkbox\" {isCompleted} " +
            $"hx-post=\"/todos/{todoItem.Id}/toggle\" hx-target=\"closest div\" " +
            "hx-swap=\"outerHTML\"/>");
        builder.AppendHtml($"<button hx-delete=\"/todos/{todoItem.Id}\"" +
            "hx-target=\"closest div\" hw-swap=\"outerHTML\">X</button>");
        builder.AppendHtml("</div>");
        return builder;
    }

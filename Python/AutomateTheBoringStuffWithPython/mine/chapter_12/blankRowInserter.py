...
blankRowStart = int(sys.argv[1])
blankRowCnt = int(sys.argv[2])
...
for row in range(1, sheetRows + 1):
    for col in range(1, sheetCols + 1):
        if row < blankRowStart:
            newSheet.cell(row, col).value = sheet.cell(row, col).value
        else:
            newSheet.cell(row + blankRowCnt, col).value = sheet.cell(row, col).value
